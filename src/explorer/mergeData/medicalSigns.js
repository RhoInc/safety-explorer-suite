...
export default function medicalSigns(dm, bds) {
    ...
    const adbds = dm.raw
        .filter(d => withResults.indexOf(d.USUBJID) < 0);
    ...
    bds.forEach(data => {
        //ADBDS variables
        const bdsVariables = Object.keys(data.raw[0])
            .filter(key => dmVariables.indexOf(key) < 0);
        const bdsVariableMapping = schema.variables
            .filter(variable => variable.sdtm.domain === 'BDS' && variable.name !== variable.sdtm.name)
            .map(variable => {
                return {
        ...
        const sdtmRenames = bdsVariableMapping
        ...
        data.raw.forEach(d => {
            ...
            for (const bdsVariable in d) {
                const variable = sdtmRenames.indexOf(bdsVariable) > -1
                    ? bdsVariableMapping
                        .find(mapping => mapping.sdtm === bdsVariable)
                        .name
                    : bdsVariable;
